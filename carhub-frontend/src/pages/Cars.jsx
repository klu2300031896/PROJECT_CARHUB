...
function Cars() {
  ...
  const searchByDate = async () => {
    if (!startDate || !endDate) {
      Swal.fire({ icon: "error", title: "Error", text: "Please choose both start and end dates", confirmButtonColor: "#d4af37", timer: 2200, showConfirmButton: false });
      return;
    }
    try {
      ...
      console.error(err);
      Swal.fire({ icon: "error", title: "Error", text: err?.response?.data?.message || "Could not load available cars", confirmButtonColor: "#d4af37", timer: 2200, showConfirmButton: false });
    }
  };
  ...
  const searchFilters = async () => {
    try {
      ...
      console.error(err);
      Swal.fire({ icon: "error", title: "Error", text: err?.response?.data?.message || "Search failed", confirmButtonColor: "#d4af37", timer: 2200, showConfirmButton: false });
    }
  };

  const bookCar = async (carId) => {
    if (!startDate || !endDate) {
      Swal.fire({ icon: "error", title: "Error", text: "Pick start and end date before booking", confirmButtonColor: "#d4af37", timer: 2200, showConfirmButton: false });
      return;
    }
    try {
      const token = localStorage.getItem("token");
            ...
            {cars.map((car) => (
              <div key={car.id} className="card" style={{ padding: 12 }}>
                <div style={{ height: 160, overflow: "hidden", borderRadius: 8, marginBottom: 10 }}>
                  <img src={car.imageUrl ? `https://carhub-backend-production-39e4.up.railway.app:8080${car.imageUrl}` : "https://via.placeholder.com/400x200?text=No+Image"} alt="car" style={{ width: "100%", height: "100%", objectFit: "cover" }} />
                </div>
                <h3 style={{ margin: "0 0 6px" }}>{car.brand} {car.model}</h3>
                <p style={{ margin: "0 0 6px", color: "#94a3b8" }}>ID: {car.id}</p>
                <p style={{ margin: "0 0 6px", color: "#cbd5e1" }}>Fuel: {car.fuelType || "N/A"}</p>
                <p style={{ margin: "0 0 8px", fontWeight: 700, color: "#f5d76e" }}>Rs. {car.pricePerDay}/day</p>
                <p style={{ margin: 0, color: car.available ? "#22c55e" : "#f97316" }}>Status: {car.available ? "Available" : "Booked"}</p>
