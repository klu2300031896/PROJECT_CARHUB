...
function Register() {
  ...
  const sendOtp = async () => {
    if (!name || !email || !password) {
      Swal.fire({ icon: "error", title: "Error", text: "Please fill name, email and password.", confirmButtonColor: "#d4af37", timer: 2000, showConfirmButton: false });
      return;
    }
    if (!validateEmail(email)) {
      Swal.fire({ icon: "error", title: "Error", text: "Enter valid email address.", confirmButtonColor: "#d4af37", timer: 2000, showConfirmButton: false });
      return;
    }
    try {
      const res = await API.post("/auth/send-otp", { name, email, password });
      Swal.fire({ icon: "info", title: "OTP Sent", text: res.data || "OTP sent to your email", confirmButtonColor: "#d4af37", timer: 2000, showConfirmButton: false });
      setShowOtp(true);
    } catch (err) {
      console.error(err);
      Swal.fire({ icon: "error", title: "Error", text: err.response?.data || "Could not send OTP", confirmButtonColor: "#d4af37" });
    }
  };

  const verifyOtp = async () => {
    if (!otp) {
      Swal.fire({ icon: "error", title: "Error", text: "Enter OTP", confirmButtonColor: "#d4af37", timer: 2000, showConfirmButton: false });
      return;
    }
    try {
      const res = await API.post("/auth/verify-otp", { name, email, password, otp });
      Swal.fire({ icon: "success", title: "Success", text: res.data || "Registration Successful", confirmButtonColor: "#d4af37", timer: 2000, showConfirmButton: false }).then(() => { window.location.href = "/"; });
      setShowSuccess(true);
    } catch (err) {
      console.error(err);
      Swal.fire({ icon: "error", title: "Invalid OTP", text: err.response?.data || "OTP verification failed", confirmButtonColor: "#d4af37", timer: 2000, showConfirmButton: false });
    }
  };
    ...
    <div className="auth-page">
      ...
      {showSuccess && (
        <div style={{ position: "fixed", inset: 0, background: "rgba(0,0,0,0.65)", display: "grid", placeItems: "center", zIndex: 50 }}>
          <div className="card" style={{ maxWidth: 380, padding: 20, textAlign: "center", border: "1px solid rgba(255,255,255,0.16)" }}>
            <div style={{ fontSize: 28, marginBottom: 10, fontWeight: 700 }}>OK</div>
            <h2 style={{ margin: "0 0 6px" }}>Registration Successful</h2>
            <p style={{ color: "#cbd5e1", marginBottom: 16 }}>You can now login with your email and password.</p>
